...
public sealed class UnityModelFileIdResolver
{
    ...
    private void AddFbxMappings(string path)
    {
        ...
        try
        {
            ...
            var authoredRoot = roots.FirstOrDefault(entry =>
                entry.Path.Count == 2 && entry.Node.Name == "root");
    ...
    {
        // These nodes are generated by Assimp for FBX pivots and pre/post rotations.
        // Unity file IDs and prefab paths address the authored nodes beneath them.
        if (UnityModelPath.IsTransformHelper(node.Name))
        {
            foreach (Node child in node.Children) CollectNodes(child, parentPath, result);
            return;
        }
