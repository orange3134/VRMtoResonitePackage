...
- **アバター候補の範囲**: 自身に VRCAvatarDescriptor がある prefab と、単一の親 prefab から Descriptor を
  継承する純粋な variant を候補にする。複数の独立 prefab を合成する外側 prefab まで再帰的に候補へ昇格すると、
  Fyuett の FaceEmo/色違い構成のようなパッケージで候補探索が爆発するため除外する。GUI には Descriptor が
  直接あるか親から継承したかを表示する。Eku_Another は単一親 variant なので引き続き候補になる。
