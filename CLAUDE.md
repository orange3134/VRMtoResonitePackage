...
- A Merge Armature source may already be prefab-parented beneath its target armature. Do not exclude descendant source candidates; select the candidate with the largest recursively matching child-bone map. `12B.prefab` places the additional `LunaWhite` armature under the primary `Armature` and otherwise rewrites zero clothing bone references.
- An unpacked/nested prefab can reference an FBX only through direct `SkinnedMeshRenderer.m_Mesh` GUIDs, with no FBX `PrefabInstance` at the leaf. FBX discovery and placement traversal must both recognize that form, carrying the placement inherited through enclosing prefab variants to the direct mesh FBX. `Fyuett_All_Hina -> FY_Chignon_Hina -> FY_Chignon -> FY_Chignon_Main -> Chignon.fbx` must keep Chignon under the avatar's `RootNode`; dropping that placement makes Merge Armature preserve the wrong global bone pose.
- When Merge Armature moves an unmatched accessory child from a source bone to its mapped target bone, preserve the child's local transform, not its global transform. The two parents represent the same semantic bone; world-pose preservation bakes source/target armature basis differences into the accessory. Fyuett Hina's `Chignon_Hair_Root` should remain near local `(0, 0.02065, 0.00649)` with identity rotation instead of becoming approximately `(0, -1.0675, -1.0925)` with a -90 degree rotation.
...
- When lilToon's baker shader is unavailable, keep the serialized main texture directly. For emission, match the SDK fallback order: `_EmissionMap`, then `_EmissionBlendMask`, then the main texture when `_EmissionMainStrength > 0`, otherwise white. Emission fallback textures also need `RGB *= A`.
- Enable lilToon outlines for dedicated Outline shader variants and for material variants that override outline properties without serializing `_UseOutline`. Avatar packages often omit lilToon's shader assets, so recognize the built-in `*_o` shader GUIDs as well as resolving included shader names. Map `_OutlineWidth` directly, matching Resonite.UnitySDK. Kipfel 1.2.0 is the regression case (`Hair`/`Outfit` use `lts_o`, while `_UseOutline` remains 0).
- Do not convert lilToon Rim to XiexeToon. Its Fresnel, border/blur, lighting, shadow mask, blend mode, and rim texture interactions cannot be represented reliably; leave `RimIntensity=0`, matching Resonite.UnitySDK. BOUKOKUNOHIME is the regression case.
