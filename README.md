...
配布版は起動時に GitHub Releases の最新版を確認します。更新がある場合は確認画面から
ダウンロード、実行ファイルの差し替え、再起動まで行えます。起動時の確認は設定画面で無効にできます。
...
### Unityプロジェクト内のPrefabを変換する

Unityプロジェクト内のVRChatアバターの `.prefab` をResoPonへドロップすると、
`.unitypackage` に書き出さずに `.resonitepackage` へ変換できます。CLIでも指定できます。

```powershell
ResoPon.exe "D:\MyUnityProject\Assets\Avatar\Avatar.prefab" -o "D:\ConvertedAvatars"
```
...
ロックファイルがない場合は `PackageCache` を読み込まず、`Assets` と埋め込み `Packages` のみを使用します。
...
UIの翻訳は `src/VrmToResonitePackage/Locales/` に言語ごとのJSONファイルとして分離されています。
新しい言語を追加するには、`en.json` をコピーして一意なBCP 47言語コード（例: `fr`、`pt-BR`）、
その言語での表示名、各文字列の翻訳を記述してください。追加したファイルはビルド時に自動で埋め込まれ、
設定画面の言語一覧に表示されます。翻訳がないキーは英語で表示されます。
...
- `Cache\` — エンジンのキャッシュ
- `Logs\` — 変換ログ。問題が起きたときはここを確認してください。GUIでは設定画面下部、または変換失敗時の「ログフォルダを開く」ボタンから開けます。
